#!/usr/bin/env python3
"""
UCASS 2 vs UCASS 6 — 10-minute scatter by wind-speed range.

Complementary to the fixed-threshold and median-split analyses (unchanged).

For each non-overlapping 10-minute interval:
  x = UCASS 2 sum in b3.1 (4.86–6.04 µm)
  y = UCASS 6 sum in b9 (4.66–6.02 µm)
  colour = wind-speed range of mean WS_ms_Avg over the same interval

Groups (half-open except upper bound of last range):
  0–2 m/s   : 0 ≤ mean wind < 2 m/s
  2–4 m/s   : 2 ≤ mean wind < 4 m/s
  4–6 m/s   : 4 ≤ mean wind ≤ 6 m/s
"""
...
OUTPUT_PNG = OUTPUT_DIR / "UCASS2_vs_UCASS6_bin_scatter_10min_by_wind_ranges.png"
OUTPUT_CSV = OUTPUT_DIR / "UCASS2_vs_UCASS6_bin_scatter_10min_by_wind_ranges.csv"
FIGURE_DPI = 300
AGG_INTERVAL = "10min"

WIND_RANGES: tuple[tuple[str, str, float, float, str], ...] = (
    ("0_2", "0–2 m/s", 0.0, 2.0, "#1f77b4"),
    ("2_4", "2–4 m/s", 2.0, 4.0, "#ff7f0e"),
    ("4_6", "4–6 m/s", 4.0, 6.0, "#2ca02c"),
)
...
def assign_wind_class(ws: float) -> str | None:
    for key, _, lo, hi, _ in WIND_RANGES:
        if key == "4_6":
            if lo <= ws <= hi:
                return key
        elif lo <= ws < hi:
...
def format_group_stats_line(label: str, fit: dict[str, float | int | str]) -> str:
    n = int(fit["n"])
    if n == 0:
        return f"{label}: no data in this wind-speed range (n = 0)"
    if np.isfinite(fit["r_squared"]):
        return (
            f"{label}: $R^2$={fit['r_squared']:.3f}, r={fit['r']:.3f}, "
            f"p={fit['p_value']:.3g}, n={n}"
        )
    return f"{label}: insufficient variation, n={n}"
...
def main() -> None:
    ...
    intervals["wind_class"] = intervals["WS_ms_Avg"].map(assign_wind_class)
    ...
    groups = [
        WindGroup(
            ...
            label,
            color,
            intervals["wind_class"] == key,
        )
        for key, label, _, _, color in WIND_RANGES
    ]
    ...
    for group in groups:
        ...
        if len(x) > 0:
            ...
            ax.scatter([], [], s=70, color=group.color, label=f"{group.label} (n = 0)")

        stats_lines.append(format_group_stats_line(group.label, fit))
    ...
    print("UCASS 2 vs UCASS 6 — 10-minute scatter by wind-speed range")
    print("=" * 60)
