#!/usr/bin/env python3
"""
UCASS 1 / 2 / 6 intercomparison — fixed non-overlapping 5-minute bins.
...
Per 5-minute interval, sum those per-second totals only when the interval
has complete triple overlap: 300 common seconds spanning the full bin.
...
BIN_LENGTH = pd.Timedelta(minutes=5)
...
OUTPUT_FIGURE = OUTPUT_DIR / "UCASS_multi_bin_intercomparison_scatter_5min.png"
OUTPUT_MASTER_CSV = OUTPUT_DIR / "UCASS_multi_bin_scatter_5min.csv"
...
def is_complete_interval(per_second: pd.DataFrame, bin_start: pd.Timestamp) -> bool:
    """True when all three instruments share every second in [bin_start, bin_start + 5 min)."""
...
def main() -> None:
    ...
    print("\nUCASS multi-bin intercomparison — complete 5-minute bins only")
    print("=" * 60)
    print(f"Per-second rows (all instruments): {len(per_second)}")
    print(f"Candidate 5-minute intervals    : {n_candidate}")
