#!/usr/bin/env python3
"""
UCASS 2 vs UCASS 6 — 10-minute scatter coloured by mean wind speed (4 m/s threshold).

Complementary to the 2 m/s fixed-threshold and median-split analyses (unchanged).

For each non-overlapping 10-minute interval:
  x = UCASS 2 sum in b3.1 (4.86–6.04 µm)
  y = UCASS 6 sum in b9 (4.66–6.02 µm)
  colour = mean wind speed over the same interval

Groups:
  Low wind  — mean wind < 4 m/s
  High wind — mean wind ≥ 4 m/s
"""
...
OUTPUT_PNG = OUTPUT_DIR / "UCASS2_vs_UCASS6_bin_scatter_10min_by_wind_4ms.png"
OUTPUT_CSV = OUTPUT_DIR / "UCASS2_vs_UCASS6_bin_scatter_10min_by_wind_4ms.csv"
FIGURE_DPI = 300
AGG_INTERVAL = "10min"
...
def format_group_stats_line(label: str, fit: dict[str, float | int | str]) -> str:
    n = int(fit["n"])
    if n == 0:
        return f"{label}: no intervals met this criterion (n = 0)"
...
def main() -> None:
    ...
    intervals["wind_class"] = np.where(
        intervals["WS_ms_Avg"] < WIND_THRESHOLD_MS,
        "lt_4",
        "ge_4",
    )
    ...
    groups = [
        WindGroup(
            "lt_4",
            f"Mean wind < {WIND_THRESHOLD_MS:.0f} m/s",
            "#1f77b4",
            intervals["WS_ms_Avg"] < WIND_THRESHOLD_MS,
        ),
        WindGroup(
    ...
    print("UCASS 2 vs UCASS 6 — 10-minute scatter by mean wind speed (4 m/s threshold)")
    print("=" * 60)
